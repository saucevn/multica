---
title: CLI Reference
description: Complete command reference for the Hira CLI and agent daemon.
---
...
When connecting to a self-hosted Hira instance, point the CLI to your server before logging in:

```bash
export HIRA_APP_URL=https://app.example.com
export HIRA_SERVER_URL=wss://api.example.com/ws
...
hira config set server_url wss://api.example.com/ws
```
