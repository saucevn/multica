...
const PREFS_PATH = join(homedir(), ".hira", "desktop_prefs.json");
...
// Keep the Go impl in sync: server/cmd/hira/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".hira", "profiles", profile)
    : join(homedir(), ".hira");
}
...
// Sidecar file that records which Hira user the cached PAT in config.json
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["hira.exe"] : ["hira"];
 ...
 *   to `apps/desktop/resources/bin/hira`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Hira.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "hira.exe" : "hira";
...
/**
 * Returns a usable `hira` binary path. Priority:
 ...
 *   5. `hira` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Hira Desktop" }),
  });
  if (!res.ok) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, HIRA_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "hira CLI is not installed" };
