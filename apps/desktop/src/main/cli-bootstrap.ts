...
// Desktop prefers the bundled `hira` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/hira-vn/hira/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "hira.exe" : "hira";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `hira-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `hira` binary. If one is already present at
