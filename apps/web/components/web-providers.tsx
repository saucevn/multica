...
// cookie mode on their next logout/login cycle (logout clears hira_token).
// Sunset: once telemetry shows <1% of sessions still carry hira_token,
...
function hasLegacyToken(): boolean {
  ...
  try {
    return Boolean(window.localStorage.getItem("hira_token"));
  } catch {
    return false;
  }
}
