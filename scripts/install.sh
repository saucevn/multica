#!/usr/bin/env bash
# Hira installer — installs the CLI and optionally provisions a self-host server.
#
...
#
# After installation, run `hira setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/hira-vn/hira.git"
REPO_WEB_URL="https://github.com/hira-vn/hira"  # without .git, for GitHub web APIs
INSTALL_DIR="${HIRA_INSTALL_DIR:-$HOME/.hira/server}"
BREW_PACKAGE="hira-vn/tap/hira"
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/hira-vn/hira/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). Hira supports macOS, Linux, and Windows." ;;
  esac
...
# ---------------------------------------------------------------------------
install_cli_brew() {
  info "Installing Hira CLI via Homebrew..."
  if ! brew tap hira-vn/tap 2>/dev/null; then
  ...
  if ! brew install "$BREW_PACKAGE" 2>/dev/null; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      ok "Hira CLI already installed via Homebrew"
    else
      fail "Failed to install hira via Homebrew."
    fi
  else
    ok "Hira CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing Hira CLI from GitHub Releases..."
  ...
  local url="https://github.com/hira-vn/hira/releases/download/${latest}/hira-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/hira.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/hira.tar.gz" -C "$tmp_dir" hira

  # Try /usr/local/bin first, fall back to ~/.local/bin
  local bin_dir="/usr/local/bin"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/hira" "$bin_dir/hira"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/hira" "$bin_dir/hira"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/hira" "$bin_dir/hira"
    chmod +x "$bin_dir/hira"
  ...
  rm -rf "$tmp_dir"
  ok "Hira CLI installed to $bin_dir/hira"
}
...
get_selfhost_ref() {
  if [ -n "${HIRA_SELFHOST_REF:-}" ]; then
    printf '%s' "$HIRA_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading Hira CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "Hira CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "Hira CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists hira; then
    local current_ver
    # `hira version` outputs "hira v0.1.13 (commit: abc1234)" — extract just the version
    current_ver=$(hira version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "Hira CLI is up to date ($current_ver)"
      return 0
    fi

    info "Hira CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(hira version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ok "Hira CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists hira; then
    fail "CLI installed but 'hira' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. Hira self-hosting requires Docker and Docker Compose.
...
setup_server() {
  ...
  info "Pulling official Hira images..."
  pull_official_selfhost_images
  info "Starting Hira services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  Hira — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ Hira CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}hira setup${RESET}                # Connect to Hira Cloud (hira.vn)\n"
  printf "     ${CYAN}hira setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/hira-vn/hira/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  Hira — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ Hira server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}hira setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/hira-vn/hira/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No Hira installation found at $INSTALL_DIR"
  fi

  if command_exists hira; then
    hira daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the Hira CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "After installation, run 'hira setup' to configure your environment."
        exit 0
        ;;
