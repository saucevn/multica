...
var rootHelpTemplate = `Work seamlessly with Hira from the command line.

USAGE
  hira <command> <subcommand> [flags]
{{range .Groups}}
{{.Title}}
...
EXAMPLES
  ...
  $ hira daemon start
  $ hira agent list --output json

ENVIRONMENT VARIABLES
  HIRA_SERVER_URL    Override the default server URL
  HIRA_WORKSPACE_ID  Set the active workspace

LEARN MORE
  Use ` + "`hira <command> <subcommand> --help`" + ` for more information about a command.
`
