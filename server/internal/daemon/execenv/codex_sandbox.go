...
// `no such host` errors when calling out (for example, `hira issue get`
// hitting the Hira API). See upstream issue openai/codex#10390.
//
...
// actually reach the Hira API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Hira CLI can reach the API.
...
// hiraManagedBeginMarker / hiraManagedEndMarker delimit the block the
...
const (
	hiraManagedBeginMarker = "# BEGIN hira-managed (do not edit; regenerated by daemon)"
	hiraManagedEndMarker   = "# END hira-managed"
)

// renderHiraManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.hira]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertHiraManagedBlock), avoids both traps.
func renderHiraManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(hiraManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(hiraManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(hiraManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(hiraManagedEndMarker) + `\n?`)

// upsertHiraManagedBlock returns the config content with the hira-managed
...
// `[permissions.hira]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderHiraManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertHiraManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderHiraManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the hira-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertHiraManagedBlock(existing, policy)
