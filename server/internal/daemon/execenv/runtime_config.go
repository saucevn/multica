...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if ctx.ChatSessionID != "" {
		...
		if ctx.AutopilotID != "" {
			fmt.Fprintf(&b, "- Run `hira autopilot get %s --output json` if you need the full autopilot configuration\n", ctx.AutopilotID)
		}
		b.WriteString("- Complete the autopilot instructions directly\n")
		b.WriteString("- Do not run `hira issue get`, `hira issue comment add`, or `hira issue status` for this run unless the autopilot instructions explicitly tell you to create or update an issue\n\n")
