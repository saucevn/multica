...
func TestBuildInvitationParams_SubjectStripsControls(t *testing.T) {
	...
	if p.Subject != "Alice đã mời bạn vào Acme trên Hira" {
...
func TestBuildInvitationParams_SubjectTruncated(t *testing.T) {
	...
	// Template: "Alice đã mời bạn vào <ws> trên Hira" (fork: Vietnamese subject).
	// ws is capped at maxSubjectFieldRunes; overall subject should also be bounded.
	// Count runes, not bytes: the Vietnamese template is multi-byte, so len() would
	// overstate the fixed part and make this bound vacuously true.
	maxExpected := len([]rune("Alice đã mời bạn vào  trên Hira")) + maxSubjectFieldRunes
