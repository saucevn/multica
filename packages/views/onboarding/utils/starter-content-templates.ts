...
export function buildWelcomeIssueText(
  ...
  const header = [
    ...
    ``,
    `[Learn how Hira works →](https://hira.vn/docs/how-hira-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in Hira**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to Hira — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Hira. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to Hira — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Hira. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to Hira — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Hira. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to Hira — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Hira. They're **exploring** what Hira can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to Hira — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Hira. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to Hira — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Hira.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in Hira are triggered when an issue has:`,
        ``,
        ...
        ``,
        `[Learn about assigning issues →](https://hira.vn/docs/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about chat →](https://hira.vn/docs/chat)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about workspaces →](https://hira.vn/docs/workspaces)`,
      ].join("\n"),
    },
  ];
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `Hira works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
        ...
        ``,
        `[Learn about members and roles →](https://hira.vn/docs/members-roles)`,
      ].join("\n"),
    });
  }
  ...
  tier2.push({
    ...
    description: [
      ...
      ``,
      `[Learn about creating agents →](https://hira.vn/docs/agents-create)`,
    ].join("\n"),
  });
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      description: [
        ...
        ``,
        `[Learn about tasks →](https://hira.vn/docs/tasks)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about the inbox →](https://hira.vn/docs/inbox)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about autopilots →](https://hira.vn/docs/autopilots)`,
      ].join("\n"),
    },
  ];
...
export function buildSelfServeSubIssues(
  ...
  // Without a runtime + an agent, nothing else in Hira works. These two
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        `1. Go to [github.com/hira-vn/hira/releases/latest](https://github.com/hira-vn/hira/releases/latest) and download the \`.dmg\` for macOS`,
        ...
        ``,
        `[Learn about runtimes →](https://hira.vn/docs/daemon-runtimes)`,
      ].join("\n"),
    },
    {
      status: "todo",
...
export function buildImportPayload({
  ...
  return {
    ...
    project: {
      ...
      description:
        "A few things to try in Hira. Work through them at your own pace.",
      icon: "👋",
    },
