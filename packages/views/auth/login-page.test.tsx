...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Hira' title", () => {
    render(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to hira/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("hira_token", "existing-jwt");
