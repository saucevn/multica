...
This guide documents the local development workflow for contributors working on the Hira codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=hira`
...
POSTGRES_DB=hira
POSTGRES_PORT=5432
DATABASE_URL=postgres://hira:hira@localhost:5432/hira?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=hira_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://hira:hira@localhost:5432/hira_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../hira-feature -b feat/my-change main
cd ../hira-feature
make dev
```
...
The daemon authenticates using the CLI's stored token (`hira login`).
...
whatever server is configured in `~/.hira/config.json`. That's fine for
...
Example: worktree at `../hira-feat-auth` produces profile
`dev-hira_feat_auth-347`, matching that worktree's port and database
allocation.
...
CONFIG_DIR="$HOME/.hira/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `hira` commands automatically use the same
...
rm -rf "$HOME/.hira/profiles/$PROFILE"
```
...
1. Compiles the `hira` CLI from `server/cmd/hira` into
   `apps/desktop/resources/bin/hira`
...
Nothing in this flow touches the system-installed `hira` or the default
`~/.hira/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.hira/config.json` | `~/.hira/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.hira/daemon.pid` | `~/.hira/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/hira_workspaces/` | `~/hira_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `hira_<slug>_<hash>` |
| Desktop profile | `desktop-api.hira.vn` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U hira -d postgres -At -c "select datname from pg_database order by datname;"
```
