...
pnpm --filter @hira-vn/views exec vitest run auth/login-page.test.tsx
pnpm --filter @hira-vn/core exec vitest run runtimes/version.test.ts
pnpm --filter @hira-vn/web exec vitest run app/\(auth\)/login/page.test.tsx
...
pnpm --filter @hira-vn/desktop build      # Compile TS → JS (reads .env.production)
pnpm --filter @hira-vn/desktop package    # Package into .app/.dmg/.exe (current platform only)
...
- `packages/ui/` — zero `@hira-vn/core` imports (pure UI, no business logic).
...
Every full-window desktop view (anything outside the dashboard shell) must mount `<DragStrip />` from `@hira-vn/views/platform` as the first flex child of the page root, otherwise users can't drag the window. Interactive UI inside the top 48px needs `WebkitAppRegion: "no-drag"` to stay clickable.
...
**Never test shared component behavior in an app's test file.** If a test requires mocking `next/navigation` or `react-router-dom` to test a component from `@hira-vn/views`, the test is in the wrong place — move it to `packages/views/` and mock `@hira-vn/core` instead.
...
- Mock `@hira-vn/core` stores with `vi.hoisted()` + `Object.assign(selectorFn, { getState })` pattern (Zustand stores are both callable and have `.getState()`).
- Mock `@hira-vn/core/api` for API calls.
